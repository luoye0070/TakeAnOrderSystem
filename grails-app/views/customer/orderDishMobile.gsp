...
<div class="panel-heading">
    <h3 class="panel-title">订单${orderInfo?.numInRestaurant}-点菜</h3>
</div>
...
<g:if test="${isOwner}">
        <div class="col-xs-4">
            <form class="" method="POST" id="cancel_form" action="${createLink(controller: "customer",action: "orderCancel")}" confirm='确定要取消订单吗？'>
        ...
        <div class="col-xs-4">
            <form class="" method="POST" id="create_form" action="${createLink(controller: "customer",action: "completeDish")}" confirm>
                    <input type="hidden" name="orderId" value="${orderInfo?.id}"/>
                    <input type="hidden" name="code" value="${params.code}"/>
...
<div>
    ...
    <ul class="breadcrumb">
        ...
        <g:else>
            <li class="active"><a href="${createLink(controller: "customer",action: "getOrCreateOrder",params: paramsT<<[foodClassId:0])}">全部</a></li>
        </g:else>
        ...
        %{--${params.foodClassId}--${foodClassInfoInstance.id.toString()}--}%
            ...
            <g:else>
                <li><a href="${createLink(controller: "customer",action: "getOrCreateOrder",params: paramsT<<[foodClassId:foodClassInfoInstance.id])}">${foodClassInfoInstance.name}</a></li>
            </g:else>
        </g:each>
    </ul>
</div>
...
<g:if test="${foodList}">
    <g:set var="backUrl" value="${createLink(controller: "customer",action: "getOrCreateOrder",params: params,absolute: true)}"></g:set>
